...
function searchResultsHTML(patients) {
  return patients.map(function (patient) {
    return '\n      <a href="/patients/' + patient._id + '" class="search__result">\n        <strong>' + patient.name + '</strong>\n      </a>\n    ';
  }).join('');
}
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/patients?q=' + this.value).then(function (res) {
      if (res.data.length) {
        searchResults.innerHTML = _dompurify2.default.sanitize(searchResultsHTML(res.data));
        return;
      }
      searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No se encontraron resultados para ' + _this.value + '</div>');
    }).catch(function (err) {
      console.error(err);
    });
  });
